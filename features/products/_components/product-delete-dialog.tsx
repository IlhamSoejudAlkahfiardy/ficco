...
export const ProductDeleteDialog: React.FC<ProductDeleteDialogProps> = ({
  ...
  const { product, invoiceUsageCount } = target;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-zinc-800 p-6 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">
              Item ini akan dihapus secara permanen dari katalog IndexedDB lokal.
            </p>
          </div>
        </div>

        {/* Identity Box */}
        <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-zinc-800/60 border border-slate-100 dark:border-zinc-800 text-xs space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-slate-500 dark:text-zinc-400">Nama Produk:</span>
          ...
          {product.sku && (
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-slate-700 dark:text-zinc-300">{product.sku}</span>
            </div>
          )}
          <div className="flex items-center justify-between">
            <span className="text-slate-500 dark:text-zinc-400">Harga Satuan:</span>
        ...
        {/* Warning if referenced in invoices */}
        {invoiceUsageCount > 0 && (
          <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 text-amber-800 dark:text-amber-300 text-xs flex items-start gap-2.5">
            ...
            <div>
              <p className="font-semibold">
                Perhatian: Digunakan pada {invoiceUsageCount} Faktur
              </p>
              <p className="mt-0.5 text-amber-700 dark:text-amber-400 leading-normal">
                Produk ini tercatat dalam baris item faktur yang telah dibuat. Faktur Anda tetap
                tersimpan dengan aman, namun referensi katalog akan terputus.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isSubmitting}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-xl bg-rose-600 hover:bg-rose-700 text-white shadow-xs transition-colors disabled:opacity-50 cursor-pointer"
          >
            {isSubmitting ? (
              <>
